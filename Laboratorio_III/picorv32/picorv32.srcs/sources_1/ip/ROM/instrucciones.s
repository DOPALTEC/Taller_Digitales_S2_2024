...
lui x19, 0x4000 //BORRAR AL IMPLEMENTAR #´s //0x40  000409B7 (Recorre la RAM en +0x4)
//addi x11, x0, 1
//lui x27, 0x0F00
//addi x27, x27, 0xD24
//addi x28, x0, 3
...
check:lw x16, 0(x4)                         //0x44  00022803 Carga el valor actual de ctrl
        and x17, x16, x10                   //0x48  00A878B3 Aisla el bit ctrl[1]
        bne x17, x0, exit_loop_num_img      //0x4C  00089A63 Sale de "check" si ctrl==1
        lw x21, 0(x7)                       //0x50  0003AA83    Carga ctrl de la tang
        and x17, x21, x10                   //0x54  00AAF8B3    Aisla ctrl[1]
        bne x17, x0, exit_loop_tang         //0x58  02089C63    Si no es igual a cero sale de "loop_tang"
        jalr x0, x0, 0x44                   //0x5C  04400067 Salta a "check"
exit_loop_num_img:
lw x20,0(x6)                                //0x60  00032A03 Carga el numero de imagen
/*
addi x19, x0, 0
lui x19, 0x4000
...
*/
loop:   
        lw x16, 0(x4)                       //0x64  00022803 (Carga el valor actual de ctrl)
        and x17, x16, x10                   //0x68  00A878B3 (Aisla el bit ctrl[1])  
        bne x17, x0, exit_loop              //0x6C  00089463 (Sale de "loop" si ctrl==1)
        jalr x0, x0, 0x64                   //0x70  06400067 (Salta a "loop")
exit_loop:lw x18, 0(x6)                     //0x74  00032903 Carga el dato recibido
sb x18, 0(x19)                              //0x78  01298023 Guarda el dato en la direccion de RAM de en un byte
addi x19, x19, 1                            //0x7C  00198993 Aumenta en 1 la direccion de byte en la RAM
beq x19, x14, imagen_escrita                //0x80  00E98463 Si todo el tamaño de imagen ya se guardo completo salta a "imagen_enviada"
jalr x0, x0, 0x64                           //0x84  06400067 Salta a "loop" si la imagen aun no se ha guardado por completo
imagen_escrita:
sw x20, 0(x3)                               //0x88  0141A023 Guarda el numero de imagen en los leds
jalr x0, x0, 0x44                           //0x8C  04400067 Salta a "check"
...
jalr x0, x0, 0x44                           //0x9C  04400067    Si el dato recibido no esta en la lista de imagenes salta a checkeo de ctrl´s[1]
